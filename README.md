...
- 客户端会把附近已经由 Xaero 完成的本机区块作为限流提示发送给服务端，服务端据此优先补齐权威瓦片；提示不包含地图像素或世界数据。
...
`tasks.dirty_chunk_scan_per_tick` 每 tick 最多检查 512 个候选区块；实际渲染受 16 块、25ms 和实时 tick 余量共同限制。瓦片压缩与原子落盘使用 2 至 4 条按坐标有序的 writer；只有落盘和索引发布成功后才确认 dirty 任务，随后主动推送到同维度客户端。Merkle 比较仅用于连接建立、丢包、断线和背压恢复，不承担持续轮询同步。
