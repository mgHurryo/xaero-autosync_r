...
当前预发布版本为 `2.0.0-beta.1`，同步协议和地图格式均为版本 2。客户端 Xaero 反射适配器固定支持：
...
- 公共路径点创建、编辑、删除、revision 冲突检测、创建者权限、数量上限和删除确认。
- 客户端管理界面：连接状态、同步开关、统计、搜索和公共路径点 CRUD。
- 中英文界面资源，默认按键 `]` 打开管理器。

## 安全边界

- 不上传世界存档、实体、容器、完整方块 NBT 或与地图显示无关的数据。
- 不自动读取或公开玩家的私人 Xaero 路径点；公共路径点只能显式创建。
...
`rebuild-loaded` 只处理在线玩家所在、当前已加载的区块，并同时更新 tile 数据缓存与索引。

## 配置

服务端首次启动生成 `config/xaero-mapsync_r.properties`，主要选项包括协议版本、最大包大小、脏区块预算、MSPT 阈值、活动阈值以及公共路径点数量和上传开关。
...
产物位于 `build/libs/`。单元测试覆盖 Merkle、tile 编解码和持久化、分片重组与重试、脏任务状态机、区域活动状态机、客户端 revision 缓存，以及固定 Xaero 版本的反射适配边界。

## 仍需发布前验证

- 使用真实 Xaero JAR 启动两个独立客户端和一个本地服务端，完成探索、离线补图、路径点 CRUD 和重启恢复的端到端验证。
- 在大型活塞/TNT/光照更新场景下采集 P95 MSPT、吞吐、带宽和缓存规模；当前只有方块变更聚合信号，没有专用 TNT、活塞和光照 Hook。
- 验证 Xaero `1.25.1` 在所有维度切换、地图打开状态和异步保存时序下的长期稳定性。
- 客户端尚未提供从私人 Xaero 路径点列表选择并显式上传的导入器；现有界面可手动创建公共路径点，且不会自动公开私人数据。
