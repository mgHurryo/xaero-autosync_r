...
Xaero 版本通过反射适配。版本或反射契约不匹配时，对应适配器会关闭，避免向未知格式写入数据。
...
| `.github/workflows/` | Gradle 构建、Xaero 参考 JAR 校验和 Qodana 静态检查。 |
| `xaeromap-origin/` | 本地联调所需的固定版本 Xaero JAR；该目录被 Git 忽略。 |

## 安装与使用

服务端安装 Fabric Loader、Fabric API 和本 Mod。需要查看共享地图或操作共享路径点的客户端还必须安装固定版本的 Xaero World Map 与 Xaero Minimap。
...
构建产物位于 `build/libs/`。当前测试树包含 51 个测试源文件和 277 个 JUnit `@Test` 用例，覆盖协议载荷、传输重组、压缩、Merkle、patch、tile 存储、客户端协调、服务端调度、权限、活动状态、dirty 队列和 Xaero 反射适配。
...
CI 会使用 JDK 21 校验 Gradle Wrapper、下载并校验固定 Xaero JAR 的 SHA-256、执行 `gradle build`，并运行 Qodana JVM 静态分析。
...
- 固定 Xaero 版本是兼容边界；升级 Xaero 前需要更新适配器并执行完整回归测试。
