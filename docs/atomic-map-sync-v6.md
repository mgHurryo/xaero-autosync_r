...
地图同步协议升级到 v10，地图数据格式保持 v6。服务端维护合并后的共享 catalog；客户端上传 Xaero 已完成写入的 tile，服务端对远处历史数据执行 fill-only 合并。共享路径点协议、存储、权限模型与界面行为不在本次变更范围内。
...
5. 客户端校验完整正方形的唯一坐标、revision 与 content hash。主波次全部校验后才按 Xaero region 统一释放；1x1/2x2 补洞包进入低优先级队列，在普通传输清空且带宽低于 50% 时逐个发送，再按 1 秒/128 包窗口合并同 region 提交，避免每个黑点触发一次 Xaero region refresh。
