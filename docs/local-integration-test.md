...
9. Restart the server and verify explored chunks, tile revisions, dirty jobs, access rules, locked waypoint identity and tombstones survive. Remove only the test server's `map_tile_index.json`, restart again, and verify the index is rebuilt from existing `.tile` files before clients connect.
