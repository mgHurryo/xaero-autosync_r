...
/** Client entry point for protocol-v8 atomic patch synchronization. */
public final class AtomicMapSyncClient {
	...
	public void handlePatchData(PatchDataPayload payload) {
		...
		MapPatchManifest expected = inFlight.remove(patch.manifest().key());
		inFlightSince.remove(patch.manifest().key());
		if (expected == null || expected.epoch() != patch.manifest().epoch()
				|| expected.contentHash() != patch.manifest().contentHash()) {
			rejectedPatches++;
		...
		if (!coordinator.enqueueVerified(patch)) {
			rejectedPatches++;
			enqueueFirst(expected);
			XaeroMapsync_r.LOGGER.warn("map_sync event=patch_deferred patch_id={} reason=commit_queue_full",
					patch.manifest().key().stableId());
		} else {
			lastProgressMillis = System.currentTimeMillis();
		}
		pumpPatchRequests();
	...
	private void expirePatchRequests(long nowMillis) {
		java.util.List<MapPatchKey> expired = inFlightSince.entrySet().stream()
				.filter(entry -> nowMillis - entry.getValue() >= PATCH_REQUEST_TIMEOUT_MILLIS)
	...
	public int pendingCount() { return requestQueue.size() + inFlight.size() + coordinator.pendingCount(); }
